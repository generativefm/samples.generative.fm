...
//const samplesIndex = require('../../public/index.json');

// s3.getObject({ Key: 'test-folder/.gitignore' })
//   .promise()
  ...
  return Promise.all(
    filenames.map(filename =>
      fs
        ...
        .then(file => gzip(file))
        .then(buffer => {
          const bucketFilename = filename.replace('public/', '');
          ...
          //     completed += 1;
          //     console.log(
          //       `${filename} upload complete (${completed}/${filenames.length})`
          //     );
          //   });
        })
    )

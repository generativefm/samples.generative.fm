      ...
      (promises, job) =>
        promises.concat(
          job.execute().then(() => {
            completedCount += 1;
            console.log(
              `${jobs.outputSample.relativePath} created (${completedCount} / ${
                jobs.length
              })`
            );
          })
...
Promise.all([
  ...
  .then(([previousIndexSamples]) => {
    const jobsToProcess = allFormatJobs.filter(({ extension, inputSample }) => {
      const instrumentSamples =
        previousIndexSamples[inputSample.instrumentName];
      return isUndefined(
        instrumentSamples ||
          instrumentSamples[extension] ||
          instrumentSamples[extension][inputSample.note]
      );
    });
